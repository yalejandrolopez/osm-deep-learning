...
parser.add_argument('--input' , type=str  , help='input geographic file', default = 'osm-data/ne.geojson')
...
# import osm dataset
labels = gpd.read_file(args.input)
...
for i in range(len(bboxes)):
    ...
    out = open(
        args.output + '/' +  str(int(bboxes[i][0])) + '_' + str(int(bboxes[i][1])) + '_' + str(int(bboxes[i][2])) + '_' + str(
            int(bboxes[i][3])) + '.tiff', 'wb')
    out.write(img.read())
    out.close()

    t2 = time.time()
    print(len(bboxes) - i, "out of ", len(bboxes), "images remaining")
    remaining_time = round((t2 - t1) / 60, 2) * (len(bboxes) - i)
    print("~", remaining_time, "min to end  \n")


try:
    os.remove(args.output+'*.xml')
except:
    pass
